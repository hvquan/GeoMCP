...
// Alternate form: "đoạn thẳng AB có trung điểm M" — segment XY has midpoint P
const midpointAltRegex = /\b([A-Z])([A-Z])\s+c[oó]\s+(trung\s*đi[eể]m|trung\s*diem|midpoint)\s+([A-Z])\b/gi;
...
// Short form: "đường cao AH" — altitude segment named by two endpoints
const altitudeShortRegex = /(đường\s*cao|duong\s*cao|altitude)\s+([A-Z])([A-Z])(?!\s*[A-Z])/gi;
...
// Short form: "đường trung tuyến AM" or "trung tuyến AM"
const medianShortRegex = /(đường\s*trung\s*tuy[eế]n|trung\s*tuy[eế]n)\s+([A-Z])([A-Z])(?!\s*[A-Z])/gi;
...
// Short form: "đường phân giác AK"
const angleBisectorShortRegex = /(đường\s*ph[aâ]n\s*gi[aá]c|ph[aâ]n\s*gi[aá]c)\s+([A-Z])([A-Z])(?!\s*[A-Z])/gi;
...
// "tam giác ... nội tiếp đường tròn (O)" — triangle inscribed in (circumscribed) circle
const inscribedInCircleRegex = /n[oộ]i\s*ti[eế]p\s*(đường\s*tr[oò]n|duong\s*tron|circle)?\s*\(?([A-Z])\)?/gi;
// "đường tròn tâm O" without radius — just a circle with a named center
const circleCenterOnlyRegex = /(đường\s*tr[oò]n|duong\s*tron|circle)\s*t[aâ]m\s*([A-Z])(?!\s*(b[aá]n|ban|,|;|\.))/gi;
...
const intersectionAtMidpointRegex = /\(?([A-Z])([A-Z])\)?\s*(va|v[aà]|and)\s*\(?([A-Z])([A-Z])\)?\s*(cat\s*nhau|c[aắ]t\s*nhau|giao\s*nhau|intersect)\s*(tai|t[aạ]i|at)\s*(trung\s*diem|trung\s*đi[eể]m|midpoint)\s*(cua|c[ủu]a|of)\s*\(?([A-Z])([A-Z])\)?/gi;
...
function pickAutoPointId(existing: string[], preferred = "M"): string {
  const used = new Set(existing.filter((p) => /^[A-Z]$/.test(p)));
  if (!used.has(preferred)) {
    return preferred;
  }

  const preferredOrder = "NPQRSTUVWXYZABCDEFGHIJKL";
...
export function parseGeometryProblem(text: string): GeometryModel {
  ...
  const equalLengths: { segment1: { a: string; b: string }; segment2: { a: string; b: string } }[] = [];
  const equalAngles: { angle1: [string, string, string]; angle2: [string, string, string] }[] = [];
  ...
  for (const source of sources) {
    for (const m of source.matchAll(midpointAltRegex)) {
      const a = pointId(m[1]);
      const b = pointId(m[2]);
      const point = pointId(m[4]);
      if (!hasPoints(point, a, b)) {
        continue;
      }
      points.push(point, a, b);
  ...
  // Short form: "đường cao AH" → altitude from A with foot H (infer base from triangle)
  for (const source of sources) {
    for (const m of source.matchAll(altitudeShortRegex)) {
      const from = pointId(m[2]);
      const foot = pointId(m[3]);
      if (!hasPoints(from, foot)) continue;
      points.push(from, foot);
      if (triangles.length > 0) {
        const tri = triangles[triangles.length - 1].vertices;
        const base = tri.filter((v) => v !== from);
        if (base.length === 2) {
          altitudes.push({ from, foot, baseA: base[0], baseB: base[1] });
          pointsOnSegments.push({ point: foot, a: base[0], b: base[1] });
          perpendiculars.push({ line1: { a: from, b: foot }, line2: { a: base[0], b: base[1] } });
        } else {
          altitudes.push({ from, foot, baseA: "", baseB: "" });
        }
      } else {
  ...
  // Short form: "đường trung tuyến AM" or "trung tuyến AM"
  for (const source of sources) {
    for (const m of source.matchAll(medianShortRegex)) {
      const from = pointId(m[2]);
      const foot = pointId(m[3]);
      if (!hasPoints(from, foot)) continue;
      points.push(from, foot);
      if (triangles.length > 0) {
        ...
        if (base.length === 2) {
          ...
          medians.push({ from, foot, baseA: "", baseB: "" });
        }
      } else {
        medians.push({ from, foot, baseA: "", baseB: "" });
      }
  ...
  // Short form: "đường phân giác AK"
  for (const source of sources) {
    for (const m of source.matchAll(angleBisectorShortRegex)) {
      const from = pointId(m[2]);
      const foot = pointId(m[3]);
      if (!hasPoints(from, foot)) continue;
      points.push(from, foot);
      if (triangles.length > 0) {
        const tri = triangles[triangles.length - 1].vertices;
        const sides = tri.filter((v) => v !== from);
        if (sides.length === 2) {
          angleBisectors.push({ from, foot, sideA: sides[0], sideB: sides[1] });
        } else {
          angleBisectors.push({ from, foot, sideA: "", sideB: "" });
        }
      } else {
  ...
  // "tam giác ... nội tiếp đường tròn (O)" → circumscribed circle (circumcircle)
  for (const source of sources) {
    for (const m of source.matchAll(inscribedInCircleRegex)) {
      const center = pointId(m[2]);
      if (!center) continue;
      points.push(center);
      circumcircles.push({ triangle: triangles.length > 0 ? triangles[triangles.length - 1].vertices : undefined });
      // Also register as a named circle
      if (!circles.some(c => c.center === center)) {
        circles.push({ center, radius: 120 });
      }
    }
  }

  // "đường tròn tâm O" without radius → circle with center O and default radius
  for (const source of sources) {
    for (const m of source.matchAll(circleCenterOnlyRegex)) {
      const center = pointId(m[2]);
      if (!center) continue;
      points.push(center);
      if (!circles.some(c => c.center === center)) {
  ...
  for (const m of normalizedText.matchAll(intersectionAtMidpointRegex)) {
    const l1a = pointId(m[1]);
    const l1b = pointId(m[2]);
    const l2a = pointId(m[4]);
    const l2b = pointId(m[5]);
    const s1 = pointId(m[10]);
    const s2 = pointId(m[11]);

    if (!hasPoints(l1a, l1b, l2a, l2b, s1, s2)) {
      continue;
    }

    const intersectionPoint = pickAutoPointId(points, "M");
    points.push(intersectionPoint, l1a, l1b, l2a, l2b, s1, s2);
    // Only add the midpoint constraint, not the intersection constraints.
    // The intersection of AD and BC at the midpoint of EH is a theorem conclusion,
    // not an input constraint. The defining property is that M is the midpoint.
    midpoints.push({ point: intersectionPoint, a: s1, b: s2 });
  }
  ...
  const raw: GeometryModel = {
    ...
    equalLengths: uniqByKey(
      equalLengths,
      (it) => `${[it.segment1.a, it.segment1.b].sort().join("")}:${[it.segment2.a, it.segment2.b].sort().join("")}`
    ),
    equalAngles: uniqByKey(
      equalAngles,
      (it) => `${it.angle1.join("")}:${it.angle2.join("")}`
    ),
    altitudes: uniqByKey(
      altitudes,
