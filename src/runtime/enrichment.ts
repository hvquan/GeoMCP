// Layer 10 — Model Enrichment
// Expands high-level constraints (altitudes, medians, named tangents, etc.)
// into the primitive constraint lists consumed by Layer 12 (constraint-solver).

import { GeometryModel, LineRef } from "../model/types.js";
...
export function enrichModelForV2(model: GeometryModel): GeometryModel {
  const enriched: GeometryModel = {
    ...
    equalLengths: [...model.equalLengths],
    equalAngles: [...model.equalAngles],
  ...
  for (const nt of enriched.namedTangents) {
    if (nt.center) {
      const circleId = enriched.circles.find((c) => c.center === nt.center)?.id
        ?? enriched.circlesByDiameter.find((c) => c.centerId === nt.center)?.circleId
        ?? nt.center;
  ...
  for (const tg of enriched.tangents) {
    ...
    const centerFromCircleId = tg.circleId
      ? enriched.circles.find((c) => c.id === tg.circleId)?.center
      : undefined;
    const circleCenter = tg.circleCenter ?? centerFromCircleId;

    if (at && circleCenter) {
      const helper = `t_${at}`;
      enriched.points.push(helper);
      enriched.perpendiculars.push({
        line1: { a: at, b: circleCenter },
        line2: { a: at, b: helper }
      });
    }
  }
